...
- [internal/sos/dashscope.go](file://internal/sos/dashscope.go)
...
- 增强环境变量覆盖机制，支持敏感信息注入
- **新增SOS语音应急系统配置**：完整的DashScope设置、FAQ语料管理、模型选择和环境变量配置
- **新增GLM-Realtime提供商支持**：新增SOSGlmConfig结构体，支持智谱GLM-Realtime语音对话，通过Provider字段在dashscope和glm之间切换
...
**更新** 本次更新重点增强了多集群连接能力、服务器安全配置、消息平台集成、事件监听功能和SOS语音应急对话能力，特别是新增了完整的SOS语音应急系统配置支持和GLM-Realtime提供商选项。
...
S3["internal/sos/dashscope.go"]
S4["internal/sos/session.go"]
end
subgraph "API服务"
A1["internal/api/middleware.go"]
A2["internal/api/sos.go"]
end
subgraph "事件系统"
...
- **SOS语音应急系统**：支持DashScope Realtime和GLM-Realtime双提供商语音对话，包含FAQ语料管理、三层兜底回答逻辑和安全会话管理。
- 服务器安全：包含API认证和CORS跨域控制中间件。
- 事件监听系统：支持Watch模式实时监听Kubernetes事件，具备速率限制和去重功能。

**更新** 新增了多集群连接、消息平台集成、SOS语音应急系统（支持双提供商）、服务器安全配置和事件监听等核心组件。
...
participant sos as "SOS语音<br/>internal/sos/*"
...
- 典型结构域
  ...
  - **SOS语音应急**：DashScope和GLM-Realtime双提供商配置、FAQ语料管理、系统提示构建
...
**更新** 新增了多集群配置、消息平台配置、SOS语音应急配置（支持双提供商）、服务器安全配置和事件监听配置。
...
### SOS语音应急配置
- **双提供商支持**
  - `provider`：语音服务提供商，支持 `dashscope`（默认）和 `glm` 两种选择
  - Provider字段大小写不敏感，会自动归一化为小写
  - 当Provider为空时默认为 `dashscope`
- **DashScope配置**
  - `enabled`：是否启用SOS语音功能
...
- **GLM-Realtime配置**
  - `glm.api_key`：智谱API密钥，格式为 `{id}.{secret}`，优先通过环境变量 `KLAW_SOS_GLM_API_KEY` 注入
  - `glm.model`：模型选择，默认 `glm-realtime`
  - `glm.voice`：可选音色字段，非空时随会话下发
- FAQ语料配置
...
**更新** 新增了完整的SOS语音应急配置，支持DashScope和GLM-Realtime双提供商语音对话、FAQ语料管理和安全的会话管理。
...
- SOS模式特定校验：确保所选提供商的必要参数完整且有效

**更新** 新增了SOS模式的配置校验，确保DashScope和GLM-Realtime连接的必要参数完整。
...
- 支持的覆盖变量
  ...
  - **`KLAW_SOS_GLM_API_KEY`**：SOS模式GLM-Realtime API密钥
- 覆盖优先级：环境变量 > 配置文件
- 安全考虑：敏感信息优先通过环境变量注入，避免落盘

**更新** 新增了SOS模式的环境变量覆盖机制，支持DashScope和GLM-Realtime API密钥的安全注入。
...
- 生产环境
  ...
  - 启用API认证和严格的CORS配置
  - 配置消息平台用于告警通知
  - 配置SOS模式用于应急语音对话（可选择DashScope或GLM提供商）
  - 使用环境变量注入所有敏感信息
- 通用最佳实践
  - 使用环境变量注入敏感信息
  - 通过配置中心或密钥管理服务集中管理
...
- 集群管理器依赖配置中的多集群设置
- 消息平台依赖配置中的平台认证信息
- **SOS语音依赖配置中的DashScope和GLM设置、FAQ语料和会话管理**
- API中间件依赖配置中的安全设置
- 事件系统依赖配置中的监听规则
...
Config --> SOS["SOS语音<br/>internal/sos/*"]
...
SOS --> DashScope["DashScope连接<br/>internal/sos/dashscope.go"]
SOS --> Session["会话管理<br/>internal/sos/session.go"]
APIMW --> SOSAPI["SOS API<br/>internal/api/sos.go"]
```
...
**更新** 新增了多集群连接、消息平台、SOS语音（支持双提供商）和API安全相关的故障排查指南，特别强调了SOS语音系统的提供商配置验证。
...
## 结论
Klaw 平台的配置管理围绕"加载—校验—使用—更新—备份"的闭环设计，兼顾灵活性与安全性。通过清晰的配置结构、严格的校验规则、可靠的多集群连接、消息平台集成、**SOS语音应急能力（支持双提供商）**、安全中间件和事件监听机制，能够在多环境下稳定运行并提供高效的运维能力。建议在生产环境中遵循最小权限、集中管理与灰度发布的最佳实践，确保配置变更的可控与可追溯。

**更新** 本次更新显著增强了平台的多集群管理能力、消息集成能力、**SOS语音应急能力（新增GLM-Realtime提供商支持）**和安全配置选项，为复杂的生产环境提供了更强大的配置支持。特别是新增的GLM-Realtime提供商选项，为企业级运维提供了更多语音交互方案选择。
