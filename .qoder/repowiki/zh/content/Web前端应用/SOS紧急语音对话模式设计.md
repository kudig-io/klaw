...
- [dashscope.go:21-56](file://internal/sos/dashscope.go#L21-L56)
...
- [dashscope.go:39-56](file://internal/sos/dashscope.go#L39-L56)
...
- [dashscope.go:21-76](file://internal/sos/dashscope.go#L21-L76)
...
**图表来源**
- [config.yaml:64-74](file://configs/config.yaml#L64-L74)
...
## 结论
SOS 紧急语音对话模式以"后端代理 + 实时全双工语音 + 三层兜底"为核心，将应急运维入口从层层页面切换到"语音即问即答"。通过严格的安全边界（密钥留在后端）、可控的工具集（只读/诊断）与可维护的语料体系（YAML + embed），在不影响现有功能的前提下，显著提升事故现场响应效率。
